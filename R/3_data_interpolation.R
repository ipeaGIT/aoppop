...
aggregate_data_to_stat_grid <- function(
  ...
  # transforming the crs significantly speeds up the operation
  #
  # suppressed warning:
  ...
  stat_grid <- sf::st_transform(stat_grid, 5880)
  tracts_with_data <- sf::st_transform(tracts_with_data, 5880)
...
# year <- 2010
# pop_unit <- tar_read(pop_units)[n_pop_unit, ]
# stat_grid <- tar_read(stat_grids_with_data, branches = n_pop_unit + ifelse(year == 2010, 0, 376))[[1]]
# hex_grid_file <- tar_read_raw(paste0("file_hex_grids_res_", res), branches = n_pop_unit)[[1]]
aggregate_data_to_hexagons <- function(
  ...
  hex_grid <- sf::st_transform(hex_grid, 5880)
  ...
  full_hexs_with_data <- sf::st_transform(full_hexs_with_data, 4674)
